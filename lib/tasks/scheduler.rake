...
task :update_feed_tuesday => :environment do
  ...
  m = Micropost.create!(content: "$4 Bud Light, well drinks, PBRs, $5 glasses of wine, other draft beers, $7 martinis, Manhattans", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("Formerly Crow's")
  u = User.find_by_name("Formerly Crow's")
  m = Micropost.create!(content: "2 for 1 well drinks, $1 off select draft beers", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("Sazón")
  u = User.find_by_name("Sazón")
  m = Micropost.create!(content: "$6 sangria, mojitos, imported beers, $2 empanadas, Mon-Fri 12pm-7pm", venue_id: v.id, user_id:u.id)
...
task :update_feed_wednesday => :environment do
  ...
  m = Micropost.create!(content: "$4 draft beers, bottled beers, $5 shots, well drinks, wines, select martinis, Mon-Fri 12pm-8pm", venue_id: v.id, user_id:u.id)
...
task :update_feed_thursday => :environment do
  ...
  m = Micropost.create!(content: "Discounted cocktails, beer and wine, complimentary hot and cold appetizers at the bar with drink purchase!", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("Crocodile Lounge")
  u = User.find_by_name("Crocodile Lounge")
  m = Micropost.create!(content: "FREE PIZZA EVERYDAY! Specials from 12pm-7pm M-F $3 Yuenglings, $4 Bud Light Draft, $1 off all other drafts, $4 well drinks, $6 Bottle Miller High Life with shot of whiskey", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("Joshua Tree")
  u = User.find_by_name("Joshua Tree")
  m = Micropost.create!(content: "$4 Yuengling, Bud, Bud LIght, $5 mixed drinks, wine, every day 12pm-8pm", venue_id: v.id, user_id:u.id)
...
task :update_feed_friday => :environment do
  ...
  m = Micropost.create!(content: "1/2 Price Bar, 11:30am-7pm Mon-Sat", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("West 3rd Common")
  u = User.find_by_name("West 3rd Common")
  m = Micropost.create!(content: "$4 draft beers, $5 wines, well drinks, cosmopolitans, appletinis, Mon-Fri 12pm-8pm", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("The House of Brews")
  u = User.find_by_name("The House of Brews")
  m = Micropost.create!(content: "$5 margaritas, cosmopolitans, appletinis, select draft beers, select bottled beers, Mon-Fri 11am-7pm", venue_id: v.id, user_id:u.id)
...
task :update_feed_saturday => :environment do
  ...
  m = Micropost.create!(content: "2 for 1 happy hour every day from 12-8pm", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("City Crab & Seafood Company")
  u = User.find_by_name("City Crab & Seafood Company")
  m = Micropost.create!(content: "Wine, beer & specialty cocktails are HALF OFF during Happy Hour! 4-7pm Monday-Friday", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("The Gin Mill")
  u = User.find_by_name("The Gin Mill")
  m = Micropost.create!(content: "1/2 Price Bar, 11:30am-8pm Mon-Sat", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("Verlaine")
...
task :update_feed_sunday => :environment do
  ...
  m = Micropost.create!(content: "$3 Bud, Bud Light, $4 Corona, Heineken, $5 well drinks, wines, $2 off domestic draft beers, every day 12pm-7pm", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("Iona")
  u = User.find_by_name("Iona")
  m = Micropost.create!(content: "$4 Stella, $3 Yuengling drafts, well drinks, $5 Smithwicks's, every day 1pm-7pm", venue_id: v.id, user_id:u.id)

  v = Venue.find_by_name("123 Burger Shot Beer")
  u = User.find_by_name("123 Burger Shot Beer")
  m = Micropost.create!(content: "$1 burgers, $2 shots, $3 beers every day 11am-8pm", venue_id: v.id, user_id:u.id)
